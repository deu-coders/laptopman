...
-- 세션 생성, 접속 권한 부여
GRANT CREATE SESSION, CONNECT TO LAPTOPMAN_GUEST;
...
BEGIN
  -- LAPTOPMAN의 모든 테이블에 대하여 SELECT 권한 부여
  FOR x IN (SELECT TABLE_NAME FROM DBA_TABLES WHERE owner = 'LAPTOPMAN')
  LOOP
    EXECUTE IMMEDIATE 'GRANT SELECT ON LAPTOPMAN.' || x.TABLE_NAME || ' TO LAPTOPMAN_GUEST_ROLE';
  END LOOP;

  -- LAPTOPMAN의 모든 프로시저에 대하여 실행 권한 부여
  FOR x IN (SELECT * FROM ALL_OBJECTS WHERE OBJECT_TYPE IN ('FUNCTION','PROCEDURE','PACKAGE') AND OWNER = 'LAPTOPMAN')
  LOOP
    EXECUTE IMMEDIATE 'GRANT EXECUTE ON LAPTOPMAN.' || x.OBJECT_NAME || ' TO LAPTOPMAN_GUEST_ROLE';
  END LOOP;
END;
